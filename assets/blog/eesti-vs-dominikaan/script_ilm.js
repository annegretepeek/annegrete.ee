...
weatherEstonia.forEach(function(d) {
	d.date = parseDate(d.Date);
});
weatherDominican.forEach(function(d) {
	d.date = parseDate(d.Date);
});
...
var angle = d3.scale.linear()
	.range([-180, 180])
	.domain(d3.extent(weatherEstonia, function(d) { return d.date; }));	 
...
textWrapper.append("text")
    ...
    .text("Temperatuur: Eesti vs Dominikaan");
textWrapper.append("text")
	.attr("class", "subtitle")
    .attr("x", 0)
...
barWrapper.append("text")
	.attr("class", "country")
    .attr("x", -4)
    .attr("y", -innerRadius * 1.03)
    .text("Eesti");
barWrapper.append("text")
	.attr("class", "country")
    .attr("x", -4)
    .attr("y", -outerRadius * 0.92)
    .text("Dominikaan");
...
barWrapper.selectAll(".tempBar")
	...
	.attr("height", function(d,i) { return barScale(d.MaxTemp) - barScale(d.MinTemp); })
 	...
 	.style("fill", function(d) { return colorScale(d.AvgTemp); });

barWrapper.selectAll(".tempBar2")
 	.data(weatherDominican)
 	.enter().append("rect")
 	.attr("class", "tempBar")
 	.attr("transform", function(d,i) { return "rotate(" + (angle(d.date)) + ")"; })
 	.attr("width", 1.5)
...
legendsvg.append("text")
	...
	.text("Päeva keskmine temperatuur");
